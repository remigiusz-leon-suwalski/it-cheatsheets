...
\item [\symbolbash] \commandbash{typeset}: alias for declare.
...
\item [\symbolbash] \commandbash{readarray}: same as mapfile.

\item [\symbolbash] \commandbash{mapfile}: reads lines into an indexed array.

\item [\symbolbash] \commandbash{readonly}: \dotfill ????

\item [\symbolbash] \commandbash{return}: stops function and returns a value.

\item [\symbolbash] \commandbash{set}: without options, prints name and value of each shell variable. Useful options are \mintinline{bash}{set -e} (exit on non-zero exit codes), \mintinline{bash}{set -o pipefail} (exit code of pipeline is last non-zero (or zero if all commands succeed)) and \mintinline{bash}{set -x} (prints commands before running them).
